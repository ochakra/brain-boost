...
export default function StatsBar() {
  ...
  const levelTitle = t.levelTitles[level.level - 1];
    ...
    <div className="bg-white border-2 border-amber-200 rounded-3xl p-5 shadow-md mb-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-bold text-gray-800 text-xl leading-tight">{levelTitle}</p>
            <p className="text-gray-500 text-base">{t.level} {level.level}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-3xl font-bold text-amber-600">{p.totalPoints}</p>
          <p className="text-gray-500 text-base">{t.points}</p>
        </div>
      </div>
      ...
      {level.nextAt !== Infinity && (
        <div className="mb-3">
          <div className="flex justify-between text-sm text-gray-500 mb-1">
            <span>{t.progressTo} {level.level + 1}</span>
            <span>{p.totalPoints} / {level.nextAt} {t.points}</span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-3">
            <div className="bg-gradient-to-r from-amber-400 to-amber-500 h-3 rounded-full transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 bg-yellow-50 rounded-2xl p-3 text-center">
          ...
          <p className="text-gray-500 text-sm">{t.badges}</p>
        </div>
      </div>
      ...
      {badges.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-2">
          {badges.map(b => (
            <div key={b.id} title={t.badgeNames[b.id]?.description} className="bg-amber-100 border border-amber-300 rounded-full px-3 py-1 flex items-center gap-1 text-sm font-semibold text-amber-800">
              {b.emoji} {t.badgeNames[b.id]?.name ?? b.name}
            </div>
          ))}
        </div>
      ) : (
        <p className="text-center text-gray-400 text-sm mt-3">{t.completeToearnBadges}</p>
      )}
    </div>
  );
}
